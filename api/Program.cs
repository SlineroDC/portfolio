...
builder.Services.AddCors(opt => opt.AddPolicy("AllowVue", policy =>
    policy.WithOrigins(allowedOrigins.Split(',')) // Convierte "url1,url2" en lista
          .AllowAnyMethod()
          .AllowAnyHeader()));
...
// === GRUPO 1: LIVE STATUS ===
app.MapGet("/api/status", async (StatusService service) =>
    Results.Ok(await service.GetCurrentStatusAsync()));

app.MapGet("/api/status/options", (StatusService service) =>
    Results.Ok(service.GetOptions()));

app.MapPost("/api/status/activate/{id}", async (int id, StatusService service, IConfiguration config, HttpRequest req) =>
{
...
app.MapPost("/api/contact", async (ContactForm form, EmailService mailer) =>
{
